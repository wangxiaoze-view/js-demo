class BasicEle extends HTMLElement {
    ...
    get itemCardTmp() {
        return utils.getEle('.card-template', false, this.contentNode);
    }

    get cardImage() {
        return utils.getEle('.card-template--img', false, this.contentNode);
    }

    get cardTitle() {
        return utils.getEle('.card-template--title', false, this.contentNode);
    }

    get cardDesc() {
        return utils.getEle('.card-template--desc', false, this.contentNode);
    }

    get cardBtn() {
        return utils.getEle('.card-template--btn', false, this.contentNode);
    }

    get style() {
        return utils.getEle('style', false, this.contentNode)
    }
...
class CustomElements extends BasicEle {
    ...
    renderPage(shadow) {
        ...
        const data = this.data.slice(this.page * this.limit, this.page * this.limit + this.limit);
        setTimeout(() => {
            let domTree = [];
    ...
    tempData() {
        ...
        for (let i = 0; i < this.max; i++) {
            arr.push({
                img: 'https://qiniu.wangxiaoze.wang/hexo-blog/default_top_img.webp', title: '标题' + i, desc: '描述' + i
            })
        }
        return arr;
    }
